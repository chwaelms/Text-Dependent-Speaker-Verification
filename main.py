...
class Task(LightningModule):
    def __init__(
        ...
        num_workers: int = 10,
        max_epochs: int = 1000,
        trial_path: str = "data/SLR_test_1m_.txt",
        checkpoint_path: str = None, # 전이 학습을 위한 사전 학습된 모델 경로
        fine_tune: bool = False, # Fine-Tuning 여부를 결정하는 인자
        **kwargs
    ):
        super().__init__()
        ...
        if self.hparams.loss_name == "amsoftmax":
            ...
            self.loss_fun = aamsoftmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes)
        else:
            self.loss_fun = softmax(embedding_dim=self.hparams.embedding_dim, num_classes=self.hparams.num_classes)
        
        print(f"Loss function initialized with num_clasees={self.hparams.num_classes}")
    ...
    def training_step(self, batch, batch_idx):
        waveform, wavform_aug, label = batch
        feature1 = self.mel_trans(waveform)
        feature2 = self.mel_trans(wavform_aug)

        embedding1 = self.encoder(feature1)
        embedding2 = self.encoder(feature2)

        alpha = 0.7 
        embedding = alpha * embedding1 + (1-alpha) * embedding2
        ...
        if batch_idx % 10 == 0:
            for name, param in self.named_parameters():
                if param.requires_grad:
                    grad_norm = param.grad.norm() if param.grad is not None else torch.tensor(0.0, dtype=torch.float32)
                    self.log(f'grad_norm_{name}', grad_norm.item())      
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        x, x_aug_list, path = batch
        path = path[0]        
        with torch.no_grad():
            x = self.mel_trans(x)
            x_aug_list = [self.mel_trans(x_aug) for x_aug in x_aug_list]
            self.encoder.eval()
            embedding = [self.encoder(x)]
            for x_aug in x_aug_list:
                embedding.append(self.encoder(x_aug))
                
            x = torch.mean(torch.stack(embedding, dim=0), dim=0)
    ...
    def validation_epoch_end(self, outputs):
        ...
        eval_vectors_cosine = eval_vectors - np.mean(eval_vectors, axis=0)
        labels, scores = score.cosine_score(
            self.trials, index_mapping, eval_vectors_cosine)
    ...
    def configure_optimizers(self):
        classifier_params = list(self.loss_fun.parameters())
        encoder_params = list(filter(
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx,
        ...
        if self.trainer.global_step < self.hparams.warmup_step:
            ...
            for idx, pg in enumerate(optimizer.param_groups):
                if idx == 0:  
                    pg['lr'] = lr_scale * self.hparams.learning_rate * 0.1
                else: 
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--train_csv_path", type=str, default="data/train_TD.csv")
        parser.add_argument("--trial_path", type=str, default="data/SLR_test.txt")
        parser.add_argument("--score_save_path", type=str, default=None)

        parser.add_argument('--eval', action='store_true')
        parser.add_argument('--aug', action='store_true')
        parser.add_argument("--fine_tune", action="store_true", help="Enable fine-tuning instead of feature extraction.")
...
def cli_main():
    ...
    # 사전 학습된 모델 로드 (pretrained model load)
    if args.checkpoint_path is not None:
        ...
        # 기존 loss_fun 출력 가중치 삭제 
        if "loss_fun.W" in state_dict:
            del state_dict["loss_fun.W"] # 기존 가중치 삭제
            # del state_dict["loss_fun.bias"]
            print("Resetting loss function weights")
            
        model.load_state_dict(state_dict, strict=False)
        print("pretrained model loaded from {}".format(args.checkpoint_path))

        # 전이 학습 : feature extractor 방식 (encoder freeze)
        if not args.fine_tune:
            for param in model.encoder.parameters():
                param.requires_grad = False
            print("Encoder is frozen. Only classification layers will be trained.")
        # fine-tuning 방식 (encoder 일부 layer 학습)
        else:
            for param in model.encoder.parameters():
                param.requires_grad = False
            ...
            for name, param in model.named_parameters():
                if any(layer in name for layer in target_layers):  # 부분 문자열 매칭
                    param.requires_grad = True
                    print(f"# Fine-Tuning Layer: {name}")  # 올바르게 선택된 경우 출력

            print("Selected layers for fine-tuning have been unfrozen.")
