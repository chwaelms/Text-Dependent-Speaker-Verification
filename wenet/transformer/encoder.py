...
from wenet.transformer.attention import MultiViewRelPositionMultiHeadedAttention
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        elif pos_enc_layer_type == "multi_view_rel_pos":
            pos_enc_class = RelPositionalEncoding
        else:
...
class ConformerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        pos_enc_layer_type: str = "multi_view_rel_pos",
        normalize_before: bool = True,
        concat_after: bool = False,
        static_chunk_size: int = 0,
        use_dynamic_chunk: bool = False,
        ...
        if pos_enc_layer_type == "no_pos":
            ...
            encoder_selfattn_layer = MultiViewRelPositionMultiHeadedAttention
        else: 
        ...
        ## 입력 데이터를 비선형 변환으로 처리
        ## macaron_style=True일 경우, 첫 번째 FFN(입력 강화)과 두 번째 FNN(추가적인 표현 학습)이 모두 활성화
        ...
        ## local 정보를 학습하여 self-attention의 한계 보완
        ## cnn_module_kernel=15로 설정되어 있어, 비교적 넓은 컨텍스트에서 지역 정보 모델링
        convolution_layer = ConvolutionModule
        convolution_layer_args = (output_size, cnn_module_kernel, activation,
                                  cnn_module_norm, causal)
        
        # (수정 2) Conv + CFA
        # channel_frequency_layer = ChannelTemporalAttention
        # channel_frequency_layer_args = (output_size, 3, 8)
        

        # 최종적으로 모든 모듈(FFN, Self-Attention, Convolution, LayerNorm) 포함
        self.encoders = torch.nn.ModuleList([
            ConformerEncoderLayer(
                ...
                # # (수정 2) Conv + CFA
                # channel_frequency_layer(*channel_frequency_layer_args) if use_cnn_module else None,
                # # (수정 2) Conv + CFA END.
                dropout_rate,
                normalize_before,
                concat_after,
