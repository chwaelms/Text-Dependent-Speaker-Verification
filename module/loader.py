import os
from typing import Any, Callable, Optional

import numpy as np
import torch
from pytorch_lightning import LightningDataModule
from torch.utils.data import DataLoader
from torch.utils.data import ConcatDataset

from pl_bolts.datasets import UnlabeledImagenet
from pl_bolts.utils.warnings import warn_missing_pkg

from .dataset import Evaluation_Dataset, Train_Dataset, Semi_Dataset


class SPK_datamodule(LightningDataModule):
    def __init__(
        self,
        train_csv_path,
        trial_path,
        unlabel_csv_path = None,
        second: int = 2,
        num_workers: int = 16,
        batch_size: int = 32,
        shuffle: bool = True,
        pin_memory: bool = True,
        drop_last: bool = True,
        pairs: bool = True,
        aug: bool = True,
        semi: bool = False,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        super().__init__(*args, **kwargs)

        self.train_csv_path = train_csv_path
        self.unlabel_csv_path = unlabel_csv_path
        self.second = second
        self.num_workers = num_workers
        self.batch_size = batch_size
        self.trial_path = trial_path
        self.pairs = pairs
        self.aug = aug
        print("second is {:.2f}".format(second))

    def train_dataloader(self) -> DataLoader:
        if self.unlabel_csv_path is None:
            train_dataset = Train_Dataset(self.train_csv_path, self.second, self.pairs, self.aug)
        else:
            train_dataset = Semi_Dataset(self.train_csv_path, self.unlabel_csv_path, self.second, self.pairs, self.aug)
        loader = torch.utils.data.DataLoader(
                train_dataset,
                shuffle=True,
                num_workers=self.num_workers,
                batch_size=self.batch_size,
                pin_memory=True,
                drop_last=False,
                persistent_workers=True if self.num_workers > 0 else False,  # (cpu 수정)
                )
        return loader

    def val_dataloader(self) -> DataLoader:
        trials = np.loadtxt(self.trial_path, str)
        self.trials = trials

        enroll_paths = set(trials.T[1])
        test_paths = set(trials.T[2])
        eval_path = np.unique(np.concatenate((list(enroll_paths), list(test_paths))))
        print("number of enroll: {}".format(len(set(trials.T[1]))))
        print("number of test: {}".format(len(set(trials.T[2]))))
        print("number of evaluation: {}".format(len(eval_path)))

        num_aug=6 # rir 증강 개수 조정 가능
        enroll_dataset = Evaluation_Dataset(list(enroll_paths), second=-1, apply_rir=True, 
                                            rir_csv_path='data/rir.csv',
                                            num_aug=num_aug)
        test_dataset = Evaluation_Dataset(list(test_paths), second=-1, apply_rir=False)
        combined_dataset = ConcatDataset([enroll_dataset, test_dataset])
        loader = torch.utils.data.DataLoader(combined_dataset,
                                             num_workers=10,
                                             shuffle=False, 
                                             batch_size=1,
                                             persistent_workers=True if self.num_workers > 0 else False,)
        return loader

    def test_dataloader(self) -> DataLoader:
        return self.val_dataloader()